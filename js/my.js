...
// メソッド化した方が良いかも？
// 変数はどっかと競合してないか？
var xml = '<xml><block type="start" deletable="false" x="400" y="50"></block></xml>';
...
function limitLoopBlock() {
  var category = document.getElementsByName("くりかえし")[0];
...
document.getElementById("sideCodeBtn").addEventListener("click", function() {
  ...
  var outputArea = document.getElementById("outputArea");
  var basicCode = outputArea.innerHTML;
  ...
  if (tab.children[0].className == "active") {
    ...
    if (sideCodeBtnClicked == "true") {
      ...
      outputAreaElement.innerHTML = basicCode;
      basicTab.appendChild(outputAreaElement);
      workspaceElement.style.width = "95%";
...
function sendKonashi(k, text) {
  ...
  var intervalId = setInterval(function() {
    console.log(sliceText,sliceText.length);
    // count++;
    k.uartWrite(new Uint8Array(new TextEncoder("utf-8").encode(sliceText[0]))).then(() => {
    ...
    progressValue = progressValue + ((1 / sliceTextLength) * 100);
    ...
    if (sliceText.length == 0) {
      ...
      // k.uartMode(Konashi.KONASHI_UART_ENABLE).then(() => {
      //   k.uartBaudRate(Konashi.KONASHI_UART_RATE_115K2).then(() => {
      //     k.uartWrite(new Uint8Array(new TextEncoder("utf-8").encode("\n"))).then(() => {
...
workspace.addChangeListener(function() {
  console.log(document.getElementById("outputArea").innerText);
